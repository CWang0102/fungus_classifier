...
# ---------------------------------------------------------------------------
# Volume – persistent storage for the model weights
# Create once:  modal volume create fungus-model-vol
# Upload weights: modal volume put fungus-model-vol ./model/fine_tuned.pth fine_tuned.pth
# ---------------------------------------------------------------------------
volume = modal.Volume.from_name("fungus-model-vol", create_if_missing=True)
...
# ---------------------------------------------------------------------------
@app.function(
    gpu="T4",                        # cheapest GPU; swap to "A10G" / "A100" if needed
    volumes={MODEL_VOLUME_PATH: volume},
    timeout=300,
    container_idle_timeout=120,      # keep warm for 2 min between requests
...
def fastapi_app():
    ...
    weights_path = Path(MODEL_VOLUME_PATH) / "fine_tuned.pth"
    ...
    def analyse_image(image_path: Path) -> ImageAnalysis:
        try:
            ...
            # Encode
            buf_orig = BytesIO()
            image_pil.save(buf_orig, format="JPEG", quality=75)
            buf_orig.seek(0)
            image_data = f"data:image/jpeg;base64,{base64.b64encode(buf_orig.read()).decode()}"

            buf_cam = BytesIO()
            Image.fromarray(overlay).save(buf_cam, format="JPEG", quality=75)
            buf_cam.seek(0)
